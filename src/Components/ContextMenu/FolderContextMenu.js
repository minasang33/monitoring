...
function FolderContextMenu(props) {
    ...
    const onFolderContextMenuAction = (event, props, action) => {
        switch (action) {
            case "new":
                //appState.actions.openModal({type: NewFolderModal, data:
              // {id: props.id, name: props.name}});
                break;
            case "rename":
                //appState.actions.openModal({type: RenameFolderModal, data:
              // {id: props.id, name: props.name}});
                break;
            case "delete":
                //appState.actions.openModal({type: DeleteFolderModal, data:
              // {id: props.id, name: props.name}});
                break;
             case "edit":
