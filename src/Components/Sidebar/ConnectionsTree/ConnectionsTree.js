    ...
    useLayoutEffect(() => {
            ...
            const nodeTitleConstructor = (node) => {
                if (node.isFolder) {
                    ...
                    return <span
                       // onDoubleClick={() =>
                      // appState.actions.activateConnection(node.id, node.text, appState.user)}
                        onContextMenu={(e) => {
        ...
        //[appState.connections, searchString, appState.actions,
      // layoutState.actions, appState.user, flattenTreeParent, getConnectionsCount, stashedExpandedKeys, expandedKeys, expandTree, expandMode.EXPAND_ALL, expandMode.EXPAND_SHALLOW]
    );
